XFubenDailyManagerCreator = function()
        ...
        for k,v in pairs(dungeonRule.SpecialConditionId)do
            if v ~= 0 then
                ...
                elseif specialCondition[v].Type == ConditionType.EventCondition then
                    stratTime = CS.XDate.GetTime(specialCondition[v].StringParam[1]) 
                    endTime = CS.XDate.GetTime(specialCondition[v].StringParam[2]) 
    ...
    function XFubenDailyManager.GetNowDayOfWeekByRefreshTime()
        ...
        local toDay = CS.XDate.GetNowLocalDayOfWeek()
        local nowTime = XTime.Now()
        local tmpTime = 0
        RefreshTime = RefreshTime or 0
        
        if RefreshTime - XTime.GetTodayTime(0,0,0) >= CS.XDate.ONE_DAY_SECOND then
            tmpTime = RefreshTime - CS.XDate.ONE_DAY_SECOND
        else
            tmpTime = RefreshTime
        end
